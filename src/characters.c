    ...
    for (u16 chr = 0; chr < MAX_CHR; chr++) {
        ...
        case STATE_HIT:
            ...
            if (obj_character[chr].modeTimer)
                ...
                talk_dialog(&dialogs[ACT1_DIALOG3][A1D3_THAT_HURTS], false); // (ES) "Eso ha dolido" - (EN) "That hurts"
                talk_dialog(&dialogs[ACT1_DIALOG3][A1D3_TRY_HIDE_OR_THUNDER], false); // (ES) "Puedo probar a esconderme|o tratar de invocar|al trueno" - (EN) "I could try to hide|or attempt to summon|the thunder"
