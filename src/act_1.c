...
void act_1_scene_2(void)    // Corridor scene with history books and memories
{
    ...
    while (true) {
        ...
        if (offset_BGA<=1 && to_int(obj_character[active_character].x)<=1) { // Players try to exit screen
            if (item_interacted[0]==false || item_interacted[1]==false) { // We han't read every book
                ...
                move_character(active_character,to_fix16(20),obj_character[active_character].y+to_fix16(obj_character[active_character].y_size)); // Go backwards
            }
    ...
    move_character(active_character,to_fix16(-30),obj_character[active_character].y+to_fix16(obj_character[active_character].y_size));
...
void act_1_scene_5(void)    // Combat tutorial scene with pattern demonstrations
{
    ...
    if (JOY_getJoypadType(JOY_1)==JOY_TYPE_PAD3) { // If three-button controller
        talk_cluster(&dialogs[ACT1_DIALOG3][A1D3_3BUTTONS]); // (ES) "Vaya, parece que estás usando|@[un mando de tres botones@]" - (EN) "Oops, it seems you are using|@[a three-button controller@]", (ES) "El juego completo necesitará|@[seis botones@], pero para esta demo|solo necesitas tres" - (EN) "The full game will need|@[six buttons@], but for this demo|you only need three"
        }
    // Fade to day palette
    ...
    move_enemy(0, to_fix16(250), to_fix16(136));
    move_enemy(1, to_fix16(20), to_fix16(156));
