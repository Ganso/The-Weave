...
void act_1_scene_1(void)    // Bedroom scene with swan's visit and pattern learning
{
    ...
    // Dialog
    talk_cluster(&dialogs[ACT1_DIALOG4][A1D4_HUNDRED_YEARS], true); // (ES) "Han pasado ya cien años" - (EN) "A hundred years have passed", (ES) "No podremos pararles|por mucho más tiempo" - (EN) "We won't be able to|stop them for much longer"
    ...
    while (scene_timeout<(SCREEN_FPS*3)) // Scene ends 3 seconds after interacting the cabinet
    {
        ...
        {
        case 0: // Bed
            talk_dialog(&dialogs[ACT1_DIALOG4][A1D4_SLEPT_BAD], false); // (ES) "He dormido regular esta noche|Tuve horribles pesadillas" - (EN) "I've not slept well last night|I had terrible nightmares"
            last_interacted_item=ITEM_NONE;
            item_interacted[0]=true;
            scene_timeout=0;
            break;
        case 1: // Chair
            talk_dialog(&dialogs[ACT1_DIALOG4][A1D4_NO_TIME_TO_SIT], false); // (ES) "No tengo tiempo de sentarme|@[Madre@] me espera" - (EN) "I don't have time to sit down|@[Mother@] is waiting for me"
            last_interacted_item=ITEM_NONE;
            item_interacted[1]=true;
            scene_timeout=0;
            break;
        case 2: // Windowsill
            talk_dialog(&dialogs[ACT1_DIALOG4][A1D4_NOT_REAL], false); // (ES) "No ha podido ser real|La ventana está cerrada" - (EN) "It couldn't be real|The windows is closed"
            last_interacted_item=ITEM_NONE;
    ...
    talk_dialog(&dialogs[ACT1_DIALOG4][A1D4_MOTHER_CALLS], true);
    talk_dialog(&dialogs[ACT1_DIALOG4][A1D4_TOO_LATE], false);
...
void act_1_scene_2(void)    // Corridor scene with history books and memories
{
    ...
    talk_cluster(&dialogs[SYSTEM_DIALOG][SYSMSG_DEMO_TITLE], true); // (ES) "@[The Weave@]|Demo técnica|Junio de 2025" - (EN) "@[The Weave@]|Tech demo|June 2025", (ES) "Los gráficos, mecánicas o sonidos|no son definitivos, ni|representan el resultado final" - (EN) "Graphics, mechanics or sounds|aren't final, nor they|represent the final result"
    ...
    // Dialog
    talk_cluster(&dialogs[ACT1_DIALOG1][A1D1_OVERSLEPT], false); // (ES) "Creo que he dormido demasiado|Debo llegar rápido al salón" - (EN) "I think I've overslept|I should go to the hall quickly", (ES) "Aunque siendo el día que es|este pasillo me trae|@[demasiados recuerdos@]" - (EN) "Although in a day like this|this hallway brings back|too @[many memories@]"
    ...
    while (true) {
        ...
        case 0: // Guild history book
            ...
            if (item_interacted[0]==false) talk_cluster(&dialogs[ACT1_DIALOG1][A1D1_BOOK_HISTORY], false); // (ES) "Este tomo narra la historia|de nuestro gremio|desde la @[Gran Separación@]" - (EN) "This volume narrates the history|of our guild|since the @[Great Split@]", (ES) "El último capítulo|termina con el fallecimiento|de mi padre" - (EN) "The last chapter|ends with the passing|of my father", (ES) "Madre dice que seré yo|el que deba escribir|el siguiente" - (EN) "Mother says it will be me|who has to write|the next one"
            else talk_dialog(&dialogs[ACT1_DIALOG1][A1D1_BOOK_HISTORY], false); // (ES) "Este tomo narra la historia|de nuestro gremio|desde la @[Gran Separación@]" - (EN) "This volume narrates the history|of our guild|since the @[Great Split@]"
            item_interacted[0]=true;
            last_interacted_item=ITEM_NONE;
            break;
        case 1: // Myths and legends
            // Read the entire text the first time, or just the first part if already read
            if (item_interacted[1]==false) talk_cluster(&dialogs[ACT1_DIALOG1][A1D1_MYTH_COLLECTION], false); // (ES) "Una colección de|mitos y leyendas|de los distintos gremios" - (EN) "A collection of|myths and legends|from the different guilds", (ES) "Gracias a mi padre|tenemos documentadas|las que cantaban los @[Pastores@]" - (EN) "Thanks to my father|we have documented|those the @[Shepherds@] sang"
            else talk_dialog(&dialogs[ACT1_DIALOG1][A1D1_MYTH_COLLECTION], false); // (ES) "Una colección de|mitos y leyendas|de los distintos gremios" - (EN) "A collection of|myths and legends|from the different guilds"
            item_interacted[1]=true;
            last_interacted_item=ITEM_NONE;
            break;
        case 7: // Left door
            talk_dialog(&dialogs[ACT1_DIALOG1][A1D1_DOOR1_CHECK], false); // (ES) "No puedo entrar en el cuarto|de los ancianos sin su permiso" - (EN) "I can't enter the elders' room|without their permission"
            last_interacted_item=ITEM_NONE;
            break;
        case 8: // Middle door
            talk_dialog(&dialogs[ACT1_DIALOG1][A1D1_DOOR2_CHECK], false); // (ES) "Luego iré a la biblioteca|Me paso allí media vida" - (EN) "I'll go to the library later|I spend half my life there"
            last_interacted_item=ITEM_NONE;
            break;
        case 9: // Right door
            talk_dialog(&dialogs[ACT1_DIALOG1][A1D1_DOOR3_CHECK], false); // (ES) "Luego volveré a mi cuarto|Ahora me esperan" - (EN) "I'll go back to my room later|Now I have to go"
            last_interacted_item=ITEM_NONE;
            break;
        case 10: // Map
            talk_dialog(&dialogs[ACT1_DIALOG1][A1D1_MAP_CHECK], false); // (ES) "Tenemos la mejor colección|de mapas de @[todos los gremios@]" - (EN) "We have the best collection|of maps from @[all guilds@]"
            last_interacted_item=ITEM_NONE;
            break;
        case 11: // Map
            talk_dialog(&dialogs[ACT1_DIALOG1][A1D1_MAP_CHECK], false); // (ES) "Tenemos la mejor colección|de mapas de @[todos los gremios@]" - (EN) "We have the best collection|of maps from @[all guilds@]"
        ...
        if (FASTFIX32_TO_INT(offset_BGA)<=1 && FASTFIX32_TO_INT(obj_character[active_character].x)<=1) { // Players try to exit screen
            if (item_interacted[0]==false || item_interacted[1]==false) { // We han't read every book
                talk_dialog(&dialogs[ACT1_DIALOG1][A1D1_REVISIT_MEMORIES], false); // (ES) "Antes de irme quiero|repasar algunos recuerdos|Se lo debo a papá" - (EN) "Before I leave I want to|revisit some memories|I owe it to dad"
...
void act_1_scene_3(void)    // Hall scene with Clio and Xander discussing Weavers
{
    ...
    // Dialog
    talk_cluster(&dialogs[ACT1_DIALOG2][A1D2_GUILD_YEAR], true); // (ES) "@[Gremio de los historiadores@]|Año 8121" - (EN) "@[Historians guild@]|Year 8121", (ES) "Lunes|Primera hora de la mañana" - (EN) "Monday|Early morning"
    move_character(CHR_linus, 200, 174);
    talk_cluster(&dialogs[ACT1_DIALOG2][A1D2_CLIO_LATE], true); // (ES) "Es tarde, Linus|Y uno no debe llegar tarde|a su cumpleaños" - (EN) "It's late, Linus|And you shouldn't be late|at your birthday", (ES) "He tenido un sueño|de lo más extraño, Madre" - (EN) "I have had the strangest|dream, Mother", (ES) "Un @[cisne@] venía a|mi cuarto y..." - (EN) "A @[swan@] came to my room|and...", (ES) "Luego me lo cuentas|@[Xander@] nos espera" - (EN) "You can tell me later|@[Xander@] is waiting for us"
    // Xander's entrance
    ...
    talk_dialog(&dialogs[ACT1_DIALOG2][A1D2_XANDER_ABILITY + response], true); // Response texts:
    ...
    look_left(CHR_clio, false);
    talk_dialog(&dialogs[ACT1_DIALOG2][A1D2_CLIO_IF_XANDER_WANTS + response], true); // Response texts:
...
void act_1_scene_5(void)    // Combat tutorial scene with pattern demonstrations
{
    ...
    // Dialog
    talk_cluster(&dialogs[ACT1_DIALOG3][A1D3_SOME_TIME_LATER], true); // (ES) "Algún tiempo después" - (EN) "Some time later", (ES) "Se aproximan enemigos|Tenemos que estar atentos|Quédate cerca, madre" - (EN) "Enemies are approaching|We have to stay alert|Stay close, mother", (ES) "@[NOTA@]: Puede que esta escena o estos|hechizos no estén en el|juego cuando esté terminado" - (EN) "@[NOTE@]: This scene or these|spells may not be in the|game when finished", (ES) "Se ha decidido incluirla|en esta demo técnica|como prueba de ciertas mecánicas" - (EN) "It's been decided to include it|in this technical demo|as a test of certain mechanics", (ES) "Pulsa @[START@] para ver|tu inventario de hechizos" - (EN) "Press @[START@] to view|your spell inventory"
    dprintf(2,"Detected controller type: %d\n", JOY_getJoypadType(JOY_1));
    if (JOY_getJoypadType(JOY_1)==JOY_TYPE_PAD3) { // If three-button controller
        talk_cluster(&dialogs[ACT1_DIALOG3][A1D3_3BUTTONS], true); // (ES) "Vaya, parece que estás usando|@[un mando de tres botones@]" - (EN) "Oops, it seems you are using|@[a three-button controller@]", (ES) "El juego completo necesitará|@[seis botones@], pero para esta demo|solo necesitas tres" - (EN) "The full game will need|@[six buttons@], but for this demo|you only need three"
        }
    // Fade to day palette
    ...
    show_or_hide_interface(false);
    talk_cluster(&dialogs[ACT1_DIALOG3][A1D3_THATS_ALL], true); // (ES) "@[¡Esto es todo!@]|(por ahora)" - (EN) "@[That's all!@]|(by now)", (ES) "Gracias por probar la demo técnica|Síguenos por X o BlueSky|@[@GeeseBumpsGames@]" - (EN) "Thanks for testing our tech demo|Follow us in X or BlueSky|@[@GeeseBumpsGames@]", (ES) "Apaga tu consola|y haz algo constructivo|como jugar un poco al frontón" - (EN) "Turn off your console|and do something constructive|like play a little racquetball", (ES) "o preparar la cena,|o organizar tu cajón de calcetines|alfabéticamente." - (EN) "or cook dinner,|or organize your sock drawer|alphabetically."
