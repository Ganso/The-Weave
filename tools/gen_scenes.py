...
OPS = {
    ...
    'next_scene':   ('SCENE_OP_NEXT_SCENE',   ['int', 'int']),
...
def parse_scene(path):
    ...
    for lineno, raw in enumerate(open(path, encoding='utf-8'), 1):
        ...
        if tok[0] == 'scene':
            scene_name = tok[1]
            if scene_name != fname:
                die(f"{where}: el nombre de escena '{scene_name}' no coincide con el archivo '{fname}'")
            continue
    ...
    m = re.fullmatch(r'act(\d+)_scene(\d+)', scene_name)
    if not m:
        die(f"{path}: el nombre debe ser actN_sceneM")
    return {'name': scene_name, 'act': int(m.group(1)), 'scene': int(m.group(2)), 'steps': steps}

# ---------------------------------------------------------------------------
# Generación
# ---------------------------------------------------------------------------
scenes = [parse_scene(p) for p in sorted(glob.glob(SCENES_GLOB))]
if not scenes:
...
with open(HEADER_FILE, 'w', encoding='utf-8') as h:
    ...
    h.write('const SceneScript* scene_lookup(u8 act, u8 scene); // NULL si no existe\n')
...
with open(SOURCE_FILE, 'w', encoding='utf-8') as c:
    ...
    c.write('const SceneScript* scene_lookup(u8 act, u8 scene)    // NULL si no existe\n{\n')
    for i, sc in enumerate(scenes):
        c.write(f"    if (act == {sc['act']} && scene == {sc['scene']}) return &scenes[{i}];\n")
    c.write('    return NULL;\n}\n')
