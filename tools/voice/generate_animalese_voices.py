...
def apply_lowpass(audio, sr, cutoff):
    """Butterworth lowpass filter — cuts bright overtones for darker timbre"""
    if cutoff <= 0 or cutoff >= sr / 2:
        return audio
    sos = butter(4, cutoff, btype='low', fs=sr, output='sos')
    return sosfilt(sos, audio).astype(np.float32)

def add_sub_harmonic(audio, sr, amount):
    """Blend a sub-octave (half frequency) copy to add chest rumble.
    
    The sub-harmonic is generated by pitch-shifting down one octave,
    lowpass-filtering it heavily (removes artifacts), and blending.
    """
    if amount <= 0:
        return audio
    sub = librosa.effects.pitch_shift(audio, sr=sr, n_steps=-12)
    # Keep only the very low end of the sub-harmonic
    sub = apply_lowpass(sub, sr, 800)
    blended = audio + sub * amount
    return blended
...
def generate_phoneme(audio, sr, params):
    ...
    if params.get("sub_harmonic", 0) > 0:
        audio = add_sub_harmonic(audio, sr, params["sub_harmonic"])
    
    if params.get("lowpass_cutoff", 0) > 0:
        audio = apply_lowpass(audio, sr, params["lowpass_cutoff"])
